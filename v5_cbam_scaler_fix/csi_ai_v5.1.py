...
#     return CR_results
"""
DROP-IN REPLACEMENT for evaluate_both_paths() in csi_ai_v5.py

Fix applied: MMSE amplitude alignment before computing PMI NMSE.
─────────────────────────────────────────────────────────────────
Root cause of positive PMI NMSE: the 3-bit quantised Type II
reconstruction (pmi_type2_reconstruct_sf) has no guarantee that its
overall amplitude matches the true channel's power — only the beam
DIRECTIONS are optimised, not the magnitude. NMSE (unlike cosine
similarity rho) is NOT scale-invariant, so any amplitude mismatch shows
up directly as inflated error power.

The fix: solve for the optimal per-sample scalar alpha that minimises
||alpha * H_hat - H_true||^2, then compute NMSE using alpha*H_hat.
This is standard practice when comparing quantised/reconstructed
channels — it isolates directional/structural reconstruction quality
from a pure amplitude-scale artefact of the quantisation scheme.

    alpha* = Re(<H_true, H_hat>) / ||H_hat||^2      (per sample)

rho (cosine similarity) is already scale-invariant, so ai_rho / pm_rho
values are UNCHANGED by this fix — only the NMSE numbers change.
"""
...
def align_scale_and_nmse(H_hat, H_true):
    ...
    alpha       = (numerator / denominator).reshape(B, 1, 1)   # broadcast
...
def evaluate_both_paths(H_norm_test, H_sf_test, args,
    ...
    PMI path:
      H_sf_test (spatial-frequency)
        → pmi_type2_reconstruct_sf → Ĥ_pmi_sf
        → MMSE amplitude-align → compare with H_sf_test
    ...
    for CR in args.cr:
        ...
        for b in range(n_batches):
            ...
            H_hat_ad = (H_hat_norm[:, 0] + 1j * H_hat_norm[:, 1]) * scaler
            H_hat_sf = angular_delay_to_sf(H_hat_ad, args.N_c)
            H_hat_sf_scaled = H_hat_sf / H_sf_scaler
            ...
            # FIX: MMSE amplitude alignment before computing NMSE.
            # rho (cosine similarity) does NOT need this — it's already
            # scale-invariant, so pm_rho is computed on the raw H_pmi_sf.
