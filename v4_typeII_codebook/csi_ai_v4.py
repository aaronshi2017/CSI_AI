...
─────────────────────────────────────────────────────────────────────────────
v4 additional fixes vs v3:
Use PMI Codebook II to match TR 38.843 Table 1 exactly.
Do some bug fixes in evaluation.    
In summary, the following bugs were fixed in v4:
-  Bug 1: H_sf normalisation for PMI path was using wrong scaler (H_sf_batch instead of H_sf_b)
-  Bug 2: PMI NMSE, SGCS, BF gain were being appended twice per batch (duplicate lines)
-  Bug 3: AI path was being compared against H_sf_b instead of H_sf_scaled (normalised)
-  Bug 4: PMI reconstruction was using W1.conj() instead of W1 for projection (Fix A)
-  Bug 5: PMI reconstruction was using only one polarisation (Fix B)
-  Bug 6: PMI reconstruction was using W1^H @ W2 instead of sum of rank-1 terms (Fix C)
-  bug 7: Beamforming gain was using H_hat instead of H_hat.conj() (Fix 2)
-  bug 8: All metrics are now computed in spatial-frequency domain (Fix 8)
-  bug 9: scalers.json is now saved with indent and verified non-empty (Fix 9)
...
def run_evaluate(args):
    ...
    # ── Plot ──────────────────────────────────────────────────────────────────
    plot_comparison(all_results, f"{MODEL_DIR}/comparison_plot_v4.png")
