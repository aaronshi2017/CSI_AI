...
─────────────────────────────────────────────────────────────────────────────
v3 additional fixes vs v2:
  use code book I do the PMI reconstruction in the spatial-frequency domain, not angular-delay domain
  Fix 8 — All metrics (NMSE, ρ, BF gain) now computed in spatial-frequency
           domain for BOTH AI and PMI paths.  This eliminates the truncation
           artefact where converting PMI output through the N_p=32 angular-
           delay bottleneck destroyed its reconstruction quality.
           Both paths are now compared against the same H_sf ground truth.
  Fix 9 — scalers.json written with indent for readability; verified non-empty
  Fix 10— mean_sc replaced with per-sample scaler lookup in evaluate

No retraining needed.  No re-generation needed.
Run:
  python csi_ai_v3.py --phase evaluate
"""
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 3 — PMI CODEBOOK  (Path B)
# ═══════════════════════════════════════════════════════════════════════════════

def build_dft_codebook(N1=8, N2=2, O1=4, O2=2):
    """3GPP TS 38.214 Type I Single-Panel DFT codebook. Returns (K, N_t)."""
    cb = []
    for m in range(N2 * O2):
        for l in range(N1 * O1):
            ...
            w = np.concatenate([np.kron(v,u)]*2) / np.sqrt(2)
            cb.append(w / np.linalg.norm(w))
    return np.array(cb)   # (128, 32)

_CB = None
def get_codebook(N_t=32):
    global _CB
    if _CB is None:
        _CB = build_dft_codebook(N1=N_t//4, N2=2, O1=4, O2=2)
    return _CB

def pmi_reconstruct_sf(H_sf_batch, N_t=32):
    """
    Vectorised PMI search + rank-1 reconstruction in spatial-frequency domain.
    H_sf_batch : (B, N_t, N_c) complex
    Returns    : (B, N_t, N_c) complex  — reconstructed H in spatial-freq domain
    """
    CB  = get_codebook(N_t)                          # (K, N_t)
    proj   = np.tensordot(CB, H_sf_batch, axes=([1],[1]))  # (K, B, N_c)
    scores = np.sum(np.abs(proj)**2, axis=2).T       # (B, K)
    best   = np.argmax(scores, axis=1)               # (B,)
    W      = CB[best]                                # (B, N_t)
    WH     = np.einsum('bn,bnc->bc', W.conj(), H_sf_batch)  # (B, N_c)
    H_hat  = np.einsum('bn,bc->bnc', W, WH)                 # (B, N_t, N_c)
    return H_hat
...
def evaluate_both_paths(H_norm_test, H_sf_test, args,
                        scaler=1.0, batch_size=500, label="All CDL"):
    """
    ...
    for CR in args.cr:
        ...
        for b in range(n_batches):
            ...
            H_sf_b  = H_sf_test[sl]     # (B,N_t,N_c)   complex — ground truth
            ...
            # Convert AI output to spatial-frequency for metric computation
            H_hat_ad = (H_hat_norm[:,0] + 1j*H_hat_norm[:,1]) * scaler  # denorm
            H_hat_sf = angular_delay_to_sf(H_hat_ad, args.N_c)           # → sf

            ai_nmse.append(nmse_db_sf(H_hat_sf, H_sf_b))
            ai_rho.append( cosine_sim_sf(H_hat_sf, H_sf_b))
            if b < 2:
                ai_bf.append(bf_gain_db_sf(H_sf_b, H_hat_sf))

            # ── Path B: PMI codebook ──────────────────────────────────────
            # Search and reconstruct directly in spatial-frequency domain
            H_pmi_sf = pmi_reconstruct_sf(H_sf_b, args.N_t)

            pm_nmse.append(nmse_db_sf(H_pmi_sf, H_sf_b))
            pm_rho.append( cosine_sim_sf(H_pmi_sf, H_sf_b))
            if b < 2:
                pm_bf.append(bf_gain_db_sf(H_sf_b, H_pmi_sf))
...
def plot_comparison(all_results, out_path):
    ...
    fig.suptitle(
        "CsiNet (AI) vs Type I DFT Codebook (PMI Baseline)\n"
...
def run_evaluate(args):
    ...
    all_results["All CDL"] = evaluate_both_paths(
        H_test, H_sf_all, args, scaler=mean_sc, label="All CDL")
    ...
    # ── Plot ──────────────────────────────────────────────────────────────────
    plot_comparison(all_results, f"{MODEL_DIR}/comparison_plot_v3.png")
