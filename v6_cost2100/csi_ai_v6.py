...
─────────────────────────────────────────────────────────────────────────────
v6 additional fixes vs v5:

DROP-IN ADDITION to csi_ai_v6.py — add this function, then in the CLI
section change:
    if args.phase in ("generate", "all"): run_generate(args)
to:
    if args.phase in ("generate", "all"): load_cost2100(args)
 
COST2100 indoor dataset (5.3 GHz pico-cell) — the standard CsiNet benchmark.
Files expected in DATA_DIR:
    DATA_Htrainin.mat
    DATA_Hvalin.mat
    DATA_Htestin.mat
 
Format inside each .mat: key "HT", shape (N, 32, 32, 2)
    dim0 = sample index
    dim1 = N_t  (32 antenna ports)
    dim2 = N_p  (32 delay taps, already truncated — CSI is pre-normalised
                 to angular-delay domain, sparse in delay dimension)
    dim3 = 2    (real, imag)
Already normalised to roughly [-1, 1] by the dataset authors — no
additional AI-side scaler is needed (scaler = 1.0).

Run:
  python csi_ai_v6.py --phase evaluate
"""
...
def load_cost2100(args):
    print("=" * 60)
    print("STEP 1 — Loading COST2100 indoor dataset")
    print("=" * 60)
    ...
    print("  Loading DATA_Htrainin.mat ...")
    H_train = load_mat("DATA_Htrainin.mat")
    print(f"    {H_train.shape}")
 
    print("  Loading DATA_Hvalin.mat ...")
    H_val = load_mat("DATA_Hvalin.mat")
    print(f"    {H_val.shape}")
 
    print("  Loading DATA_Htestin.mat ...")
    H_test = load_mat("DATA_Htestin.mat")
    print(f"    {H_test.shape}")
 
    # Save in the standard pipeline format (same filenames v5/v4 used)
    np.save(f"{DATA_DIR}/H_train.npy", H_train)
    np.save(f"{DATA_DIR}/H_val.npy",   H_val)
    np.save(f"{DATA_DIR}/H_test.npy",  H_test)
 
    # ── Build spatial-frequency test set for the PMI baseline path ──────────
    # COST2100 data is already in angular-delay domain, truncated to N_p=32.
    # The original full bandwidth for this dataset is N_c=1024 subcarriers
    # (per the CsiNet paper / CRNet convention). Zero-pad and inverse
    # transform to recover an approximate spatial-frequency H for PMI search.
    N_c_full = 1024
    print(f"\n  Building spatial-frequency test set (N_c={N_c_full}) for PMI path...")
 
    # COST2100 real/imag channels are affine-normalised to [0,1] by the
    ...
    np.save(f"{DATA_DIR}/H_sf_test_all.npy", H_sf_test.astype(np.complex64))
    print(f"    H_sf_test shape: {H_sf_test.shape}")
 
    # COST2100 is pre-normalised by the dataset authors — no additional
    # scaler needed for the AI path (scaler=1.0). Save this so run_evaluate
    # picks it up via load_scalers() without modification.
    with open(f"{DATA_DIR}/scalers.json", "w") as f:
        json.dump({"COST2100_indoor": 1.0}, f, indent=2)
 
    print(f"\nDone.")
    print(f"  Train: {H_train.shape}   Val: {H_val.shape}   Test: {H_test.shape}")
    print(f"  Data directory: {DATA_DIR}/")
    print(f"\nNote: N_c for COST2100 is {N_c_full} (vs 64 for the CDL synthetic")
    print(f"  generator in v4/v5). If args.N_c defaults to 64, pass")
    print(f"  --N_c 1024 on the train/evaluate commands too, or the")
    print(f"  angular_delay_to_sf() calls in evaluate_both_paths will")
    print(f"  zero-pad to the wrong width.")
...
class ConvBN(nn.Module):
    def __init__(self, ic, oc, k=3, p=1):
        super().__init__()
        groups = min(4, oc)  # GroupNorm needs oc divisible by groups
        self.b = nn.Sequential(nn.Conv2d(ic,oc,k,1,p,bias=False),
                               nn.GroupNorm(groups, oc), nn.LeakyReLU(0.1, True))
    def forward(self, x): return self.b(x)
...
def run_train(args):
    ...
    for CR in args.cr:
        ...
        opt   = optim.Adam(model.parameters(), lr=1e-3, weight_decay=0.0)  # decay removed: was grinding all params to 0 at CR=4 once loss plateaued
        sch   = optim.lr_scheduler.CosineAnnealingLR(opt, args.epochs, eta_min=1e-5)
        best, patience, ctr = float("inf"), 4, 0
...
def align_scale_and_nmse(H_hat, H_true):
    """
    MMSE-optimal amplitude alignment, then NMSE in dB.
    Fixes positive PMI NMSE — the Type II quantised reconstruction has
    no guarantee its overall amplitude matches the true channel power;
    only beam DIRECTION is optimised. rho (cosine similarity) is already
    scale-invariant and needs no such correction — only NMSE does.

        alpha* = Re(<H_true, H_hat>) / ||H_hat||^2      (per sample)

    H_hat, H_true : (B, N_t, N_c) complex arrays
    Returns       : float, NMSE in dB after per-sample optimal scaling
    """
    B = H_hat.shape[0]
    H_hat_flat  = H_hat.reshape(B, -1)
    H_true_flat = H_true.reshape(B, -1)

    numerator   = np.sum((H_true_flat * H_hat_flat.conj()).real, axis=1)
    denominator = np.sum(np.abs(H_hat_flat) ** 2, axis=1) + 1e-8
    alpha       = (numerator / denominator).reshape(B, 1, 1)

    H_hat_aligned = H_hat * alpha
    err = np.mean(np.abs(H_hat_aligned - H_true) ** 2, axis=(1, 2))
...
def evaluate_both_paths(H_norm_test, H_sf_test, args,
    ...
    for CR in args.cr:
        ...
        for b in range(n_batches):
            ...
            # Replace with:
            # Recentre [0,1] sigmoid output to [-1,1] before treating as a
            ...
            pm_nmse.append(align_scale_and_nmse(H_pmi_sf, H_sf_scaled))
...
def plot_comparison(all_results, out_path):
    ...
    fig.suptitle(
        ...
        "COST2100 Indoor — metrics in spatial-frequency domain — 3GPP TR 38.843",
...
def run_evaluate(args):
    ...
    # ── Plot ──────────────────────────────────────────────────────────────────
    plot_comparison(all_results, f"{MODEL_DIR}/comparison_plot_v6.png")
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser(description="CSI Feedback AI vs PMI — v3.0")
